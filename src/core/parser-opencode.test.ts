...
import { parseOpenCodeSessions, parseOpenCodeSessionsFromDb, findOpenCodeDbPaths } from './parser-opencode';
...
function createTestDb(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'opencode-sqlite-test-'));
  ...
  for (const s of sessions) {
    insertSession.run(s.id, s.slug ?? '', s.directory ?? '', s.title ?? '', s.time_created ?? 0, s.time_updated ?? 0);
  }
  ...
  for (const m of messages) {
    insertMessage.run(m.id, m.session_id, m.time_created ?? 0, JSON.stringify(m.data));
  }
  ...
  for (const p of parts) {
    insertPart.run(p.id, p.message_id, p.session_id, p.time_created ?? 0, JSON.stringify(p.data));
  }

  db.close();
  return { dbPath, cleanup: () => fs.rmSync(tmpDir, { recursive: true, force: true }) };
}
...
describe('parseOpenCodeSessionsFromDb', () => {
  it('parses a session with user + assistant messages from SQLite', () => {
    const { dbPath, cleanup } = createTestDb(
      [{ id: 'ses_1', directory: '/home/user/myproj', title: 'Test session', time_created: 1700000000000, time_updated: 1700000005000 }],
      [
        {
          id: 'msg_u1', session_id: 'ses_1', time_created: 1700000001000,
          data: { role: 'user', time: { created: 1700000001000 }, agent: 'build', summary: { title: 'hello world' } },
        },
        {
          id: 'msg_a1', session_id: 'ses_1', time_created: 1700000002000,
          data: {
            role: 'assistant', parentID: 'msg_u1',
            time: { created: 1700000002000, completed: 1700000003000 },
            modelID: 'claude-sonnet-4', providerID: 'anthropic',
            tokens: { input: 500, output: 80, reasoning: 0, cache: { write: 0, read: 200 } },
          },
        },
      ],
      [
        { id: 'prt_1', message_id: 'msg_a1', session_id: 'ses_1', time_created: 1700000002500, data: { type: 'text', text: 'Hello!' } },
      ],
    );
  ...
  it('extracts tool usage and edited files from tool parts', () => {
    const { dbPath, cleanup } = createTestDb(
      [{ id: 'ses_2', directory: '/home/user/proj2', time_created: 1700100000000 }],
      [
        {
          id: 'msg_u2', session_id: 'ses_2', time_created: 1700100001000,
          data: { role: 'user', time: { created: 1700100001000 }, summary: { title: 'edit a file' } },
        },
        {
          id: 'msg_a2', session_id: 'ses_2', time_created: 1700100002000,
          data: {
            role: 'assistant', parentID: 'msg_u2',
      ...
      [
        {
          id: 'prt_tool', message_id: 'msg_a2', session_id: 'ses_2',
          data: { type: 'tool', tool: 'write', state: { status: 'completed', input: { filePath: 'src/index.ts', content: 'const x = 1;' } } },
        },
        {
          id: 'prt_read', message_id: 'msg_a2', session_id: 'ses_2',
          data: { type: 'tool', tool: 'read', state: { status: 'completed', input: { filePath: 'src/utils.ts' } } },
        },
      ],
    );
  ...
  it('skips malformed message.data and part.data rows without crashing', () => {
    const { dbPath, cleanup } = createTestDb(
      [{ id: 'ses_3', directory: '/home/user/proj3', time_created: 1700200000000 }],
      [
        // Good message
        {
          id: 'msg_u3', session_id: 'ses_3', time_created: 1700200001000,
          data: { role: 'user', time: { created: 1700200001000 }, summary: { title: 'ok' } },
        },
        { id: 'msg_a3', session_id: 'ses_3', time_created: 1700200002000,
          data: { role: 'assistant', parentID: 'msg_u3', time: { created: 1700200002000, completed: 1700200003000 }, modelID: 'claude-haiku', tokens: { input: 10, output: 5 } } },
      ],
      [
        // Good part
        { id: 'prt_good', message_id: 'msg_a3', session_id: 'ses_3', data: { type: 'text', text: 'result' } },
      ],
    );

    // Inject a malformed row directly via better-sqlite3
    const db = new Database(dbPath);
    db.prepare('INSERT INTO message (id, session_id, time_created, data) VALUES (?, ?, ?, ?)').run('msg_bad', 'ses_3', 1700200000500, 'NOT JSON {{{');
    db.prepare('INSERT INTO part (id, message_id, session_id, time_created, data) VALUES (?, ?, ?, ?, ?)').run('prt_bad', 'msg_a3', 'ses_3', 1700200002100, '{invalid}');
    db.close();
  ...
  it('handles a large number of sessions without throwing', () => {
    ...
    const messages = sessions.flatMap(s => [
      { id: `msg_u_${s.id}`, session_id: s.id, time_created: s.time_created + 1, data: { role: 'user', time: { created: s.time_created + 1 }, summary: { title: `prompt ${s.id}` } } },
      { id: `msg_a_${s.id}`, session_id: s.id, time_created: s.time_created + 2, data: { role: 'assistant', parentID: `msg_u_${s.id}`, time: { created: s.time_created + 2, completed: s.time_created + 3 }, modelID: 'claude-haiku', tokens: { input: 50, output: 10 } } },
    ]);
...
describe('findOpenCodeDbPaths', () => {
  it('returns a DB path when HOME points to a dir with opencode.db', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'opencode-discovery-test-'));
  ...
  it('returns empty array when no opencode.db exists', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'opencode-discovery-empty-'));
    const origHome = process.env.HOME;
    const origUserProfile = process.env.USERPROFILE;
